...
export function MessageBubble({ message, isMe }: MessageBubbleProps) {
  ...
  const likeIcon = myLike ? <FavoriteIcon sx={{ height: 25, color: "lightblue" }} /> 
    : likes.length > 0 ? <FavoriteIcon sx={{ height: 25, color: "#fff" }} /> 
    ...
    <Box display="flex" flexDirection="column" alignItems={isMe ? "flex-end" : "flex-start"} mb={2} gap={0.5}>
      {!isMe ? (
        <Stack direction="row" alignItems="center" spacing={1} mb={0.5}>
          ...
          <Typography fontSize={13} color="#ccc">{message.sender.firstName} {message.sender.lastName}</Typography>
          <Typography fontSize={11} color="#aaa" sx={{ userSelect: "none", pt: 0.25, whiteSpace: "nowrap" }}>{timestamp}</Typography>
        </Stack>
      ) : (
        <Stack direction="row" alignItems="center" justifyContent="flex-end">
          <Typography fontSize={11} color="#aaa" sx={{ userSelect: "none", pb: 0.5, whiteSpace: "nowrap" }}>{timestamp}</Typography>
        </Stack>
      )}
      <Stack direction="row" alignItems="flex-end" spacing={0.7} justifyContent={isMe ? "flex-end" : "flex-start"} width="50%">
        <Paper elevation={2} sx={{ maxWidth: "90%", p: 1.3, background: isMe ? "#1976d2" : "#2a2a2a", color: "white", borderRadius: 3 }}>
          {editing ? (
            <Stack direction="row" spacing={1}>
              <Input
                value={editValue}
                onChange={(e) => setEditValue(e.target.value)}
                style={{ padding: 6, background: "#111", color: "white", borderRadius: 6, border: "1px solid #444", width: "100%" }}
              />
              <IconButton size="small" onClick={handleUpdate} sx={{ color: "lightblue" }}>
                <Edit />
              </IconButton>
              <IconButton size="small" onClick={() => { setEditValue(message.content); setEditing(false); }} sx={{ color: "red" }}>
                <CloseIcon />
              </IconButton>
            </Stack>
          ) : (
      ...
      <Stack direction="row" alignItems="center" spacing={1.2} mt={0.2} sx={{ opacity: 0.9, width: "100%", justifyContent: isMe ? "flex-end" : "flex-start" }}>
        {isMe && (
          <>
            ...
            <Popover
              ...
              PaperProps={{ sx: { backgroundColor: "#1e1e1e", minWidth: 200, padding: "5px 0", border: "1px solid #444" } }}
            >
              <Stack spacing={1}>
                <MenuItem onClick={() => { setEditing(true); setEditValue(message.content); setMenuAnchor(null); }} sx={{ display: "flex", justifyContent: "space-between", color: "white" }}>
                  Update <Edit sx={{ color: "lightblue", height: 20 }} />
                </MenuItem>
                <MenuItem onClick={() => { handleDelete(); setMenuAnchor(null); }} sx={{ display: "flex", justifyContent: "space-between", color: "white" }}>
        ...
        <Stack direction="row" alignItems="center" spacing={0.3}>
          <IconButton size="small" onClick={handleToggleLike} sx={{ p: 0.3 }}>{likeIcon}</IconButton>
          <Typography fontSize={13} color={likeColor}>{likes.length}</Typography>
        </Stack>
        {isMe && readCount > 0 && (
          <Stack direction="row" alignItems="center" spacing={0.3} ml={1}>
            <VisibilityIcon sx={{ fontSize: 17, color: "#9ecbff" }} />
            <Typography fontSize={12} color="#9ecbff">{readCount}</Typography>
          </Stack>
        )}
      </Stack>
    </Box>
